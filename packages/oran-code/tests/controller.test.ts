...
import type { Message, ModelConfig, ModelProvider, ModelResponse, ModelStreamChunk, RuntimeEvent, ToolCall, ToolDefinition } from "../src/types.js";
...
describe("TaskController", () => {
  ...
  it("streams, invokes a tool, verifies, and persists the trace without a plan step", async () => {
    ...
    try {
      ...
      const call: ToolCall = { id: "call_1", name: "write_note", arguments: { content: "hello" }, createdAt: new Date().toISOString() };
      const provider = new FakeProvider([
        toolResponse(call),
        textResponse("Done. The note was written."),
      ]);
      const events: RuntimeEvent[] = [];
      const { controller, trace } = createController(workspace, provider, registry, events, () => true);
  ...
  it("records a rejected approval and still lets the model finish", async () => {
    ...
    try {
      ...
      const events: RuntimeEvent[] = [];
      const { controller, trace } = createController(workspace, provider, registry, events, (call) => call.name === "__plan__");
