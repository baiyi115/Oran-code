...
describe("SkillLoader", () => {
  it("uses ~/.agents/skills as the canonical user Skill directory", async () => {
    ...
    const loader = new SkillLoader(workspace, { builtin: join(workspace, "builtin"), project: join(workspace, "project"), legacyUser: false });
  ...
  it("scans the legacy user directory before the canonical user directory", async () => {
    ...
    expect((await fixture.loader().scan())[0]).toMatchObject({ name: "shared", description: "canonical", scope: "user" });
  });
  ...
  it("loads strict file and directory skills with project-over-user-over-builtin precedence", async () => {
    ...
    await writeSkill(join(fixture.project, "shared", "SKILL.md"), skillDocument("shared", "project", {
  ...
  it("installs an HTTP definition and rejects unsafe names before writing", async () => {
    const fixture = await createFixture();
    const loader = fixture.loader();
    vi.stubGlobal("fetch", vi.fn(async () => new Response(skillDocument("remote", "from HTTP"))));
  ...
  it("uses an explicit install name or the source filename when frontmatter omits a name", async () => {
    ...
    expect(await readFile(join(fixture.project, "renamed-skill", "SKILL.md"), "utf8"))
      .toContain("name: renamed-skill");
  });
});
...
describe("skill execution adapters", () => {
  it("renders arguments and fails fast when required tools are absent", () => {
    ...
    expect(renderSkillPrompt(definition({ body: "Review carefully" }), "src/index.ts"))
      .toBe("Review carefully\n\n## User request\n\nsrc/index.ts");

    expect(() => assertSkillTools(definition({ allowedTools: ["read_file", "shell_command"] }), ["read_file"]))
      .toThrow("shell_command");
  ...
  it("registers inline and derived commands with markers while preserving conflicts", async () => {
    ...
    await writeSkill(join(fixture.project, "review.md"), skillDocument("review-skill", "derived skill", {
      mode: "derived",
      context: "recent",
    }));
    ...
    expect(registry.get("/review-skill")).toMatchObject({ kind: "isolated-skill", description: "derived skill [skill]" });
