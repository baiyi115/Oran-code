...
export interface SessionMessageRecord extends Message {
  type: "message";
  timestamp: string;
}
export interface LegacySessionMessageRecord extends Message {
  timestamp?: string;
  type?: undefined;
}
export interface SessionCompactionBoundaryContent {
  summary?: string;
...
export interface SessionCompactionBoundaryRecord {
  ...
  role: "system";
  timestamp: string;
  content: SessionCompactionBoundaryContent;
}
...
export interface SessionTaskPlanRecord {
  type: "task-plan";
  timestamp: string;
  plan: TaskPlanState;
}
export type SessionJsonlRecord =
  ...
  | LegacySessionMessageRecord
  | SessionCompactionBoundaryRecord
  | SessionResetRecord
...
export class SessionStore {
  ...
  async open(): Promise<void> {
    ...
    const valid = entries.filter(
      (item): item is { session: StoredSession; mtimeMs: number; conversationLoaded: boolean } => item !== undefined,
    );
  ...
  list(): StoredSession[] {
    return this.sessions
      .filter((session) => normalizeWorkspace(session.workspace) === normalizeWorkspace(this.workspace))
      .sort(
        (a, b) => (this.mtimes.get(b.id) ?? 0) - (this.mtimes.get(a.id) ?? 0) || b.updatedAt.localeCompare(a.updatedAt),
      )
      .map(cloneSession);
  }
  ...
  find(id: string): StoredSession | undefined {
    const found = this.sessions.find(
      (item) => item.id === id && normalizeWorkspace(item.workspace) === normalizeWorkspace(this.workspace),
    );
  ...
  async create(
    ...
    defaults: Pick<
      StoredSession,
      "workMode" | "permissionMode" | "reasoningEffort" | "modelReference" | "conversation" | "planState"
    > = {},
    ...
    const session: StoredSession = {
      id,
      name: normalizedName,
      autoNamed: DEFAULT_SESSION_NAMES.has(normalizedName),
    ...
    session.archiveSize = await this.persistRecordsAndState(id, initialRecords, session, now);
  ...
  async update(id: string, patch: StoredSessionPatch): Promise<StoredSession | undefined> {
    ...
    const planRecords: SessionJsonlRecord[] = patch.planState ? [planRecord(patch.planState, now)] : [];
    const conversationChanges: SessionJsonlRecord[] =
      patch.conversation === undefined
        ? []
        : isMessagePrefix(before, after)
          ? messageRecords(after.slice(before.length), now)
          : [
              compactionBoundaryRecord(after, now) ?? {
                type: "session-reset",
                role: "system",
                timestamp: now,
                content: { messages: cloneMessages(after) },
              },
            ];
    ...
    if (existing.archiveMessageCount !== undefined || existing.conversation !== undefined) {
      next.archiveMessageCount =
        (existing.archiveMessageCount ?? countArchiveMessages(before)) +
  ...
  private async refreshMtime(id: string): Promise<void> {
    try {
      this.mtimes.set(id, (await stat(this.archivePath(id))).mtimeMs);
    } catch {
  ...
  private async readStateSnapshot(id: string): Promise<SessionStateRecord | undefined> {
    try {
      ...
      return isSessionJsonlRecord(parsed) && parsed.type === "state" && parsed.session.id === id ? parsed : undefined;
    } catch {
      return undefined;
    }
  }
  ...
  private async readMetadata(
    path: string,
    fileId: string,
  ): Promise<{ session: StoredSession; mtimeMs: number; conversationLoaded: boolean } | undefined> {
    try {
      const [details, snapshot] = await Promise.all([stat(path), this.readStateSnapshot(fileId)]);
      if (
        snapshot?.session &&
        isStoredSessionState(snapshot.session) &&
        snapshot.session.id === fileId &&
  ...
  private async loadConversation(id: string): Promise<void> {
    ...
    if (previous.titleGenerationAttempted !== undefined)
      merged.titleGenerationAttempted = previous.titleGenerationAttempted;
  ...
  private async readArchive(
    path: string,
    fileId: string,
  ): Promise<{ session: StoredSession; mtimeMs: number } | undefined> {
    try {
      ...
      const latestRecord =
        matchingSnapshot && (!archiveState || matchingSnapshot.timestamp >= archiveState.timestamp)
          ? matchingSnapshot
          : archiveState;
      const latest = latestRecord?.session;
      const effectivePlan = latest?.planState ?? latestPlan;
      const base: StoredSession | undefined =
        latest && isStoredSessionState(latest) && latest.id === fileId
          ? {
      ...
      if (compactArchive || refreshSnapshot) {
        try {
          ...
          /* loading remains valid even if best-effort migration fails */
        }
      }
  ...
  private async migrateLegacyStore(): Promise<void> {
    const candidatePaths = [this.path, resolve(this.sessionsRoot, "sessions.json")].filter(
      ...
      try {
        ...
        for (const session of legacy) {
          ...
          const records: SessionJsonlRecord[] = [
            stateRecord(migrated, timestamp),
            ...messageRecords(migrated.conversation ?? [], timestamp),
          ];
          await this.persistRecordsAndState(session.id, records, migrated, timestamp);
        }
        await rename(legacyFile, `${legacyFile}.migrated`);
      } catch {
        /* retain the legacy file so migration can be retried */
      }
    }
  }
}
...
export function parseSessionJsonl(contents: string): SessionJsonlRecord[] {
  ...
  for (const line of contents.split(/\r?\n/)) {
    ...
    try {
      ...
      if (isSessionJsonlRecord(value)) records.push(value);
    } catch {
      /* skip a damaged line without losing the archive */
    }
  }
  return records;
}
...
export function rebuildConversation(records: readonly SessionJsonlRecord[]): Message[] {
  ...
  for (let index = records.length - 1; index >= 0; index -= 1) {
    ...
    if (type === "compaction-boundary" || type === "session-reset") {
      boundaryIndex = index;
      break;
    }
  }
...
function recordMessage(record: SessionJsonlRecord | undefined): Message | undefined {
  if (
    !record ||
    record.type === "state" ||
    record.type === "compaction-boundary" ||
    record.type === "session-reset" ||
    record.type === "task-plan" ||
    !isMessageRole(record.role)
  )
    return undefined;
  const hasCalls = Array.isArray(record.toolCalls) && record.toolCalls.length > 0;
  if (!(typeof record.content === "string" && record.content.trim()) && !hasCalls && record.role !== "tool")
    return undefined;
...
function isMessagePrefix(prefix: readonly Message[], complete: readonly Message[]): boolean {
  return (
    ...
    prefix.every((message, index) => JSON.stringify(message) === JSON.stringify(complete[index]))
  );
}
function compactionBoundaryRecord(
  messages: readonly Message[],
  timestamp: string,
): SessionCompactionBoundaryRecord | undefined {
  let summaryIndex = -1;
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    if (messages[index]?.metadata?.promptBlock === "context-summary") {
      summaryIndex = index;
      break;
    }
  }
  ...
  const retainedTail = messages
    .slice(summaryIndex + 1)
    .filter(
      (message): message is Message & { role: "user" | "assistant"; content: string } =>
...
function isSessionJsonlRecord(value: unknown): value is SessionJsonlRecord {
  ...
  if (item.type === "compaction-boundary")
    return item.role === "system" && typeof item.timestamp === "string" && isBoundaryContent(item.content);
  if (item.type === "session-reset")
    return item.role === "system" && typeof item.timestamp === "string" && isResetContent(item.content);
  if (item.type === "task-plan") return typeof item.timestamp === "string" && isTaskPlanState(item.plan);
  return (
    (item.type === undefined || item.type === "message") &&
    isMessageRole(item.role) &&
    (item.timestamp === undefined || typeof item.timestamp === "string") &&
    isPersistedMessage(item)
  );
}
function isBoundaryContent(value: unknown): value is SessionCompactionBoundaryContent {
  ...
  const validSummary =
    ...
    (item.retainedTail === undefined ||
      (Array.isArray(item.retainedTail) && item.retainedTail.every(isRetainedMessage)));
...
function isStoredSession(value: unknown): value is StoredSession {
  return (
    isStoredSessionState(value) &&
    ((value as Partial<StoredSession>).conversation === undefined ||
      Array.isArray((value as Partial<StoredSession>).conversation))
  );
}
function isStoredSessionState(value: unknown): value is StoredSessionState {
  ...
  return (
    ...
    item.messages.every(isTranscriptMessage) &&
    Array.isArray(item.history) &&
    item.history.every((entry) => typeof entry === "string") &&
    ...
    (item.titleSource === undefined ||
      item.titleSource === "local" ||
      item.titleSource === "model" ||
    ...
    (item.archiveMessageCount === undefined ||
      (typeof item.archiveMessageCount === "number" && Number.isFinite(item.archiveMessageCount))) &&
...
function isTranscriptMessage(value: unknown): value is TranscriptMessage {
  ...
  if (item.kind === "user")
    return typeof item.text === "string" && (item.queued === undefined || typeof item.queued === "boolean");
  ...
  if (item.kind === "tool") {
    return (
      ...
      typeof item.permissionLevel === "number" &&
      typeof item.expanded === "boolean" &&
