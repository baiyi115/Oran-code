...
export interface MemoryNoteMetadata { readonly id: string; readonly type: MemoryNoteType; readonly description: string; }
export interface MemoryNote extends MemoryNoteMetadata { readonly body: string; readonly path: string; readonly scope: MemoryScope; readonly modifiedAt: string; }
export interface MemoryWriteInput extends MemoryNoteMetadata { readonly body: string; readonly fileName?: string; }
export interface MemoryManagerOptions {
  readonly userDirectory?: string; readonly projectDirectory?: string; readonly maxIndexLines?: number;
  readonly maxIndexBytes?: number; readonly maxCandidates?: number; readonly maxRelevant?: number;
}
export interface FindRelevantOptions { readonly injectedIds?: Iterable<string>; readonly maxCandidates?: number; readonly maxResults?: number; }
...
export class MemoryManager {
  ...
  async scan(): Promise<MemoryNote[]> {
    const [project, user] = await Promise.all([this.scanDirectory(this.projectDirectory, "project"), this.scanDirectory(this.userDirectory, "user")]);
    return [...project, ...user];
  }
  ...
  async writeNote(input: MemoryWriteInput): Promise<MemoryNote | undefined> {
    ...
    return { id: normalized.id, type: normalized.type, description: normalized.description, body: normalized.body, path, scope, modifiedAt: fileStat.mtime.toISOString() };
  }
  ...
  async findRelevant(query: string, provider: ModelProvider, options: FindRelevantOptions = {}): Promise<MemoryNote[]> {
    try {
      ...
      const candidates = (await this.scan()).filter((note) => !injected.has(note.id))
        .sort((left, right) => right.modifiedAt.localeCompare(left.modifiedAt)).slice(0, maxCandidates);
      if (!candidates.length) return [];
      const response = await provider.complete([
        { role: "system", content: SELECTOR_SYSTEM_PROMPT },
        { role: "user", content: [`Query:\n${query.trim()}`, "Candidates:", ...candidates.map((note) => [
          `- id: ${note.id}`, `  scope: ${note.scope}`, `  type: ${note.type}`, `  path: ${this.displayPath(note)}`,
          `  modified: ${note.modifiedAt}`, `  description: ${note.description}`,
        ].join("\n"))].join("\n") },
      ]);
  ...
  private async scanDirectory(directory: string, scope: MemoryScope): Promise<MemoryNote[]> {
    ...
    const notes = await Promise.all(files.map(async (path): Promise<MemoryNote | undefined> => {
...
export function normalizeMemoryNoteType(value: unknown): MemoryNoteType | undefined {
  ...
  const aliases: Record<string, MemoryNoteType> = {
    "user-preference": "user-preference", preference: "user-preference", "用户偏好": "user-preference",
    "correction-feedback": "correction-feedback", correction: "correction-feedback", feedback: "correction-feedback", "纠正反馈": "correction-feedback",
    "project-knowledge": "project-knowledge", project: "project-knowledge", "项目知识": "project-knowledge",
    "reference-material": "reference-material", reference: "reference-material", "参考资料": "reference-material",
  };
...
function normalizeWriteInput(input: MemoryWriteInput): MemoryWriteInput | undefined {
  const id = input.id.trim(); const description = input.description.trim(); const body = input.body.trim();
...
function safeMarkdownFileName(value: string): string {
  ...
  const safe = withoutExtension.normalize("NFKC").replace(/[<>:"/\\|?*\u0000-\u001f]/gu, "-").replace(/\.\.+/gu, "-")
    .replace(/\s+/gu, "-").replace(/-+/gu, "-").replace(/^[. -]+|[. -]+$/gu, "").slice(0, 96);
  return `${safe || `memory-${randomUUID().slice(0, 8)}`}.md`;
}

function resolveInside(directory: string, name: string): string {
  const root = resolve(directory); const target = resolve(root, name);
...
async function atomicWrite(path: string, content: string): Promise<void> {
  ...
  catch { await unlink(path).catch(() => undefined); await rename(temporary, path); }
  finally { await unlink(temporary).catch(() => undefined); }
}

function boundLines(lines: readonly string[], maxLines: number, maxBytes: number): string {
  const accepted: string[] = []; let bytes = 0;
  for (const rawLine of lines) for (const line of rawLine.split(/\r?\n/u)) {
    ...
    accepted.push(line); bytes += nextBytes;
  }
...
function parseSelectedIds(value: string): string[] {
  ...
  const objectStart = normalized.indexOf("{"); const arrayStart = normalized.indexOf("[");
...
function compareById(left: MemoryNote, right: MemoryNote): number { return left.id.localeCompare(right.id) || left.path.localeCompare(right.path); }
